...
export default function FileManager({ folders, currentFolderId, currentFolder, files }: Props) {
  ...
  const confirmDeleteFolder = () => {
    ...
    router.delete(`/media/${deleteFolderId}`, {
      ...
      onSuccess: () => {
        ...
        if (deletingCurrent) {
          router.visit('/admin/files', { replace: true });
        } else {
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    router.post('/admin/files', formData, {
      forceFormData: true,
      preserveScroll: true,
      onSuccess: () => {
    ...
    return nodes.map((folder) => (
      <div key={folder.id} style={{ marginLeft: level * 12 }} className="mb-1">
        <div
          ...
          onClick={() => router.visit(`/admin/files?folder_id=${folder.id}`)}
        >
        ...
        <div className="md:col-span-1 border rounded-lg p-4 bg-white">
          ...
          <div className="overflow-y-auto max-h-[calc(100vh-250px)]">
            <div
              ...
              onClick={() => router.visit('/admin/files')}
            >
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction
                              onClick={() => router.delete(`/admin/files/${file.id}`, {
                                preserveScroll: true,
                                onSuccess: () => {
                                  setPreviewFile(null);
                                  router.reload({ only: ['files'] });
